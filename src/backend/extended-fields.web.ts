...
export async function POST(request: Request) {
    ...
    try {
        ...
        console.log('=== CRM Extended Fields Query ===');
        console.log('Requested field keys:', fieldKeys);
        ...
        console.log('Total fields in CRM:', allFields.length);

        // Log available fields for debugging
        console.log('Available CRM fields:');
        allFields.forEach(field => {
            console.log(`  ${field.key} -> "${field.displayName}" (${field.fieldType})`);
        });
        ...
        relevantFields.forEach(field => {
            console.log(`  Matched: ${field.key} -> "${field.displayName}"`);
        });
        ...
        console.error('Error in extended-fields API:', error);
        return new Response(
            JSON.stringify({
...
export async function OPTIONS(request: Request) {
    ...
    console.log('CORS preflight request received from:', request.headers.get('origin'));
    console.log('Applied CORS headers:', corsHeaders);
